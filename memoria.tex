...
% El pensamiento computacional parece estar exclusivamente relacionado con la informática y las ciencias de la computación pero es más un proceso cognitivo~\ref{moreno19:_programming} que puede ser desarrollado a partir de la programación, pero también a partir de otras actividades ``desenchufadas''. En nuestro caso, se analiza a partir de dos lenguajes de programación visual en bloques. Permite resolver problemas y comunicar ideas aprovechando la potencia que ofrecen los ordenadores
...
% , researchers in all disciplines including the arts, humanitiesand social sciences—are discovering new knowledge using computational methods and tools.
...
%Seleccionar, manipular, filtrar, merge, melt, groupby, pivot agrupar datos to effectively store, manipulate, and gain insight from data. vander
...
% Programas cada vez más sofisticados, desde un proceso tinkering, montar las piezas como con Lego, bloques, imágenes y sonido unidos, testean si el comportamiento de los personajes es el esperado. Scratch in Practice: Start with Exploration, Not Explanation Natalie Rusk
Como lenguaje de programación Scratch se planteó conforme a 3 objetivos: \emph{``low floor, high ceiling, wide walls''}~\cite{resnick2009scratch}. Es sencillo iniciarse en la código-alfabetización a través de Scratch, puesto que los bloques presentan formas que indican cómo deben unirse y están organizados conforme a su funcionalidad en 10 categorías con colores distintos: movimiento, apariencia, sonido, eventos, control, sensores, operadores, variables y mis bloques. Esta última categoría fue añadida en la versión 2.0 y permite definir procedimientos o bloques propios, que junto a las listas y variables suponen un nivel superior de abstracción, para crear proyectos cada vez más complejos y modulares. Así el aprendiz moldea su aprendizaje, adquiere destrezas computacionales y da rienda suelta a su imaginación escogiendo, en un entorno diverso los casos de uso, extensiones y temas que mejor se adapten a sus inquietudes, vagaje y raíces culturales. En la figura~\ref{fig:road_trip} se observa el editor de Scratch con uno de los proyectos analizados en este trabajo de fin de grado. %múltiples posibilidades
%encontrando el apoyo de la comunidad, en estudios, grupos y foros.
%tinkering, desarrollar fragmentos de conocimiento que pueden integrar más tarde en un entendimiento completo. el proceso de abajo a arriba. Tinkerer proceso continuo. Importancia del proceso más que en el resultado final, que planean hacer luego,lleva su tiempo esta fase de tinkering mientras exploran nuevas ideas, tiempos extendidos de proyectos...
...
% interfaz de scratch facilita la depuración del código, pues permite ver que partes se están ejecutando, para detectar posibles errores, y los valores actuales de los datos almacenados en variables y listas, cambios de estado, Puedes ver los cambios de estado de variables y listas en tiempo real, Entender como los datos se relacionan con el programa.
% Diferentes tipos de bloques para animaciones lápiz y efectos de imágenes, bloques de decir y pensar para historias, bloques de detectar colisiones entre objetos e interacciones con el teclado para juegos, pen blocks para que los personajes dibujen mientras se mueven arte interactivo, simulaciones con operaciones matemáticas, bloques de notas, instrumentos, y tempo para música, pensar de manera divergente. Hacer proyectos con personajes que se mueven e interactúan. Estrategias y fuentes de inspiración

% fomentar, animar, alentar, estimular, incentivar, incitar alfabetización literacy, engage invulucrarse comprometerse, involucrados y motivados, regular sus emociones
...
% Se canalizan reunir gente con intereses comunes o complementarios, Colab Camps
% amplia gama de proyectos inspiradores e iniciales
% procesos de pensamiento, pensar en proyectos, pensar en problemas
% 
%  en la hora del código como parte de la educación informales, 
% La adopción del lenguaje de programación de Scratch en las etapas educativas obligatorias o K-12 de numerosos países como forma para adquirir la competencias computacionales 
% evitar problemas con la sintaxis de lenguajes textuales
% interaccionan en la web comparten interactúan y aprenden los unos de los otros docentes y estudiantes comunidad, modificaciones, trayectorias 
% multidisciplinar, enfoque creativo y aprender expresar o comunicar ideas, pensamiento crítico...
% mujeres y estudiantes de colectivos minoritarios 
% literatura científica
% Papert argued that programming languages should have a low floor (easy to get started with) and a high ceiling (opportunities for increasingly complex projects over time). In
...
% engaged). Satisfying the triplet of low-floor/high-ceiling/wide-walls hasn’t been easy.
%  
% Hard Fun gente que se desafía a sí misma y experimenta con ideas
% Lenguaje de programación visual con bloques que permite la implementación desde el navegador web. 
% 
% No largas explicaciones o enseñanza de difusión. Menos aprendizaje broadcast de difusión, más interactivo
% 
% Bloques, personajes u objetos, eventos -->ceil hi
% 
% Expresar ideas a la comunidad expresarse a través del código y con la comunidad
% 
% Creatividad en la búsqueda de diferentes soluciones a un mismo problema... Lenguaje de programación gratuito y comunidad online
% Aprendizaje a través del juego, lúdico
% 
% en sus propias ideas, e intereses, inspiración, estudios para agrupar por temáticas o por cursos, creativa, cuidada comunidad colaboradora
% 
% Dar la oportunidad a los niños herramienta gratuita, low code, expresarse a la comunidad, creadores aprendices, studios online, usados en escuelas de todo el mundo
% expresar ideas, apoyando diversidad y la equidad
% 
% En todas las partes del curriculum, en todas las clases historia, artes, lengua y literatura, filosofía
% Coder-Dojo, Hora del Código está organizada por Code.org, Intef
% 
% Versiones de Scratch que incluyen diferencias de bloques BYOB,, el marco conceptual,,, Formato de los  proyectos,. Usado por educadores... Obligatorio a partir del proximo año el PC
% Version 3.0 en vez de trabajar con flash, fue reimplementada en JavaScript con el lenguaje de marcado HTML5 que permite un diseño responsivo adaptable a tablets e incluso a móviles. Scratch Desktop 

\section{Dr. Scratch}
\label{sec:dr_scratch}

Dr. Scratch\footnote{\url{http://www.drscratch.org}} es una aplicación web open-source que analiza proyectos programados en Scratch a través de su url o cargando el proyecto desde el ordenador. Los proyectos de la versión de Scratch 3.0 son almacenados en formato comprimido .sb3. Ya sea obteniendo el fichero de los servidores de Scratch o cargando el fichero JSON contenido en el directorio comprimido, analiza dicho fichero que contiene el código fuente del proyecto de Scratch e indica, a partir de los bloques utilizados en cada objeto, cuáles son las destrezas del pensamiento computacional demostradas en los proyectos. De manera automática el alumno, profesor u organización consiguen realimentación para mejorar en las áreas en las que menor puntuación obtenga, así como de las malas prácticas cometidas~\cite{moreno2015dr} a la hora de programar, pudiendo hacer un seguimiento de su progreso.

%Falta de consenso en definir CT, retroalimentación
Ante la ausencia de herramientas que automatizaran la evaluación del pensamiento computacional, Gregorio Robles y Jesús Moreno se propusieron desarrollar la heramienta web libre de código abierto (FLOSS) \texttt{Dr. Scratch} en 2014. Inspirada por Scrape y tomando como base los plugins de Hairball, se extendió su funcionalidad para evaluar la calidad del código de los proyectos de Scratch y poder establecer el dominio del PC, tomando como referencia los métodos de evaluación de otros investigadores y las recomendaciones del profesorado en los talleres organizados~\cite{moreno2015dr}. % Validez ecológica, convergente y  discriminante. Para la versión alpha contaron con la colaboración de David Chushig
...
% sitio web, página, servicio web, distribuida
...
\begin{table}
    ...
    \end{tabular}}
    \caption{Métricas de los niveles de competencia en cada dimensión del PC con Dr. Scratch}
...
%Las páginas que visualiza el usuario de Dr. Scratch en su navegador serán distintas, en función del lenguaje escogido, si está autenticado como programador, profesor u organización dedicada a la investigación. También se adapta al dominio demostrado en el PC al analizar estáticamente los proyectos.
% básico, en desarrollo y competente
%Problemas de estructura  Se ofrecen enlaces con una explicación del concepto y ejemplos de como mejorar dicha habilidad
%Pylint
% Solicita el JSON a los repositorios/servidores del MIT. cargando el proyecto desde el ordenador. 
%Dr. Scratch is a web application that enables teachers and students to automatically analyze projects created with Scratch, the most used programming language in primary and secondary education. The tool provides feedback aiming to support learners to improve their code and develop their computational thinking skills. Dr. Scratch is used %worldwide by students, educators, goverments, researchers and non-prift organizations to evaluate and assess programming interventions.% The tool is developed by the KGBL3 research group. jemole
%Web platform that allows to upload Scratch programs, a visual programming language oriented to the learning of programming for children, to be evaluated. Learners will receive feedback on the quality of the program, how to improve it and a global assessment of their programming capabilities. The platform includes gamification techniques, so that students can earn medals and develop their programming skills and achieve higher levels of complexity and quality. KGB-L3
%Enlace para el análisis de proyectos de Scratch .sb o .sb2
%Extensión de la funcionalidad de Hairball, nombres por defecto de personajes y escenarios, código duplicado, código muerto o inalcanzable

%FLOSS (Free/Libre Open Source Software) movements, like sharing and contributing to the community.
...
%operational CT models that can be empirically validated 
%evalúa mejor las prácticas computacionales para evaluar y crear
...
%Versión alpha, versión beta , adaptación a proyectos de Scratch .sb3

\section{Snap!}
\label{sec:snap}

Snap!\footnote{\url{https://snap.berkeley.edu/}} es un lenguaje de programación visual con bloques cuya primera versión fue desarrollada en 2008 por Jens Mönig y Brian Harvey. Denominada en sus orígenes \emph{BYOB, ``Build Your Own Blocks''}, extendía la funcionalidad de la versión 1.4 de Scratch incluyendo la posibilidad de definir procedimientos o funciones propias. Se puede elegir la categoría a la que pertenecen de las 8 disponibles y si el bloque definido, encapsulado como una primitiva, será utilizado por el objeto donde se declara o por todos los objetos del proyecto. % , diferenciadas por colores
Estos bloques propios tienen forma de comando, si es una instrucción o procedimiento, de reportero cuando es una función que devuelve un valor o de condición si devuelve un booleano. Por ello permite la recursividad, como la que se aplica en el proyecto de Snap! de la figura~\ref{fig:fractal}. % De predicado valor booleano
...
%función anónima --> map, keep, find, combine, call
...
% permiten tratar los scripts como datos
...
Como muestra de la relevancia que adquiere en Snap! este paradigma, tanto su logo como su mascota Alonzo, el antiguo Gobo de Scratch, incluyen una $\lambda$ en honor al cálculo lambda de Alonzo Church.% Empleada en la IA
...
%%Fue entonces cuando pasó a denominarse Snap!
%las continuaciones de primera clase permiten crear  multihilos a partir del contexto.
%Profundiza en los conceptos de ámbito local y global de las variables a partir de las continuaciones y los closure....
%Continuación dejar la función donde la dejaste...
%más abstracción, bloques como datos , IA ciencias de datos
%
%Programación funcional, aunque la visualización gráfica hace que se empuje hacia el lado imperativo
%Modelo de ejecución multihilos empuja a variables globales
%lenguaje de programación declarativa no es posible 
%diseño descendente top-down, no puedes llamar a una función que no has creado todavía.
%
%Snap! extiende las funcionalidades de Scratch, 
%se crean procedimientos funciones propios de cada una de las categorías, pudiendo elegir para que sea de tipo instrucción, stac devuelva una variable que sea entrada para listas, o booleana. oval, hexágono u otras funciones o las mismas
%valor devuelto por la función sea un elemento de la lista, una variable
%
%Podemos convertir nuestros proyectos en ejecutables, xml, citilab
%
%FrontEnd JS con html5 uso en móviles con android e iphone
% ámbitos de las variables persistencia de ámbito local
%
% Mantiene las primitivas de Scratch
% if then else reporter
% 
% macros, upvars, ... librerias JavaScript para nºs grandes. Ámbito léxico pero también ámbito dinámico. Fundamentos de Snap! Se apoyan en el libro SICP.
% 
%intercambio de mensajes entre objetos
%dile al sprite
%pregunta al sprite por
%
%variables propias de los objetos hijos y nietos, métodos propios
%
%Toma de Scratch la capacidad de representar ideas poderosas mediante metáforas visuales para ideas avanzadas
%Toman fuciones y listas como parámetros
%
%
%Programación de lo objetos que conformen nuestro proyecto
%izquerda instruciones, zona central para programar, dcha Stage ver la ejecución in situ
% Interfaz gráfica similar, entorno de ejecución, archivos multimedia
% Maloney
% 
%creatividad, abstracción data  impacto global
% 
% Fomentar al igual que Scratch la inclusión de los colectivos minoritarios, apostar por la diversidad. 
% Hay librerías y posibilidad de implementar código en otros lenguajes
% 
% en situaciones socioeconómicas desfavorables.
% Elegir los proyectos en función de sus preferencias...
% %https://forum.snap.berkeley.edu/t/which-is-better-snap-or-scratch/5286
% Aplicables a ciencia de datos
...
\begin{table}
    ...
    \end{tabular}}
    \caption{Métricas de los niveles de competencia en cada dimensión del PC con Dr. Snap!}
...
%aplicación web framework Django(3.1.2) desplegada en PythonAnywhere (cuenta caducada) Python 3.8.5
%
%FrontEnd HTML, CSS, JS
%BackEnd servidor, base de datos, aplicación
%Desplegar la aplicación web en PythonAnywhere servicio de hosting web en la nube.
%El siguiente paso es crearte un entorno virtual y descargar Django. 
%Estuvo accesible 3 meses, Proporciona un servidor para poder subir tu aplicacion web a la nube. Es compatible con
%proyectos de Django y existen dos modalidades: de pago y gratuita. La de pago tiene más ventajas y opciones como es la posibilidad de elegir el dominio, al contrario que la versión gratuita.
%que te impone el dominio %http://prodriguezmartinez.pythonanywhere.

%https://cloud.snap.berkeley.edu/static/API emplea granja de Servidores NGINX. DDBB: PostgreSQL 
% broadcast enviar mensaje a todos Scratch y aquí enviar mensaje a un objeto en particular?? Clones mismo comportamiento heredado del objeto padre, con atributo posición distinta creados dinámicamente hasta que pare la ejecución del programa. es una aptitud cognitiva.
...
% Semi- interpretado --> .pyc, .pyo, imperativa estructurada y funcional, módulos o paquetes, amplísima comunidad que contribuyen. Python for everybody
%dinámico y fuertemente tipado, con influencias de Lisp en 1994...
% Existen una serie de buenas prácticas. Hacer que el código sea más pitónico. 
...
%
%La versatilidad que ofrece en la elección del lenguaje de programación que ejecuta el código de las celdas.
%
%%con extensión .ipynb. aunque se intercambia la información con el servidor en local en formato JSON.
%
%Ejecución con kernels de múltiples lenguajes (40). Su nombre surge de la unión de los 3 lenguajes, Julia, Python y R.
%
%Es una aplicación web distribuida cliente-servidor que crea un entorno de programación interactiva
%una interfaz gráfica interactiva en el lado del navegador para la shell de IPython, para mostrar contenido enriquecido.
%Trazabilidad  y entorno reproducible. Guarda seguimiento de la sesión.
%Programación agnóstica interactiva científica y de uso intensivo de datos.
%Interfaz de usuario basado en la web, desarollado por el proyecto Jupyter, entorno de desarrollo interactivo para que los ciéntíficos de datos trabajen con datos ,,, kernel de ipython encargado de ejecutar el código de las celdas raw Nb convert

%% https://jupyter-notebook.readthedocs.io/en/latest/notebook.html
% representación multimedia enriquecida
% desarrollo, documentación
% Los cuadernos API web + documento
% Entorno interactivo mejorado
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Dr. Scratch Está alojado en una máquina virtual en Google Cloud Platform (PaaS) con sistema operativo Linux 18.04
...
% Proceso de análisis con muestras pequeñas.
% Meter las gráficas de la presentación.
...
\begin{table}[H]
    ...
    \begin{tabular}{|m{3.5cm}|m{5cm}|m{5cm}|m{5cm}|}
       ...
       \textbf{Condicionales} & 1 estructura selectiva: if o if-else u operaciones lógicas. & 2 estructuras selectivas: if, if - else u operaciones lógicas. & Se utilizan las 3 estructuras condicionales. \\ \hline %sile
       ...
       Eventos con el ratón sobre el objeto.%ratón pulsado
...
% evitar solapamiento entre grupos.
...
%No conocemos como podrían clasificarse los proyectos,
%Modelo para extraer patrones en los datos.
%Minería de datos
%Generar info a partir de los grupos que se conformen cuando se instancie el algoritmo
...
% Por los proyectos que contienen y por las valoraciones recibidas
...
% lo más distantes posibles en regiones probables con densidad de proyectos para disminuir el nº de iteraciones del algoritmo y evitar agrupaciones deficientes. Hacer que converga más rápido y no en un mínimo local.
...
%Equivalente al proyecto representativo del clúster 2.
...
%A partir de las etiquetas asignadas  podemos saber a qué grupo pertenece cada proyecto. 
%Se estudian las similitudes y diferencias entre los grupos del clustering con k=8 y los grupos del clustering con k=15.Para ello se analiza cuántos proyectos de los clústeres de k=8 terminan en cada grupo del clustering con k=15.
...
% múltiples hilos paralelismo, cada script ejecuta su propio hilo
...
% Cual es el impacto de utilizar cada herramienta en la asimilación del proceso cognitivo a través de las habilidades inherentes

%Se descubren y sintetizan tendencias e información relevante que ayudan a discernir las similitudes y diferencias entre ambas herramientas de aprendizaje

%Estudio comparativo del análisis de datos realizado sobre las dimensiones del pensamiento computacional demostradas en los proyectos de Scratch y Snap! almacenados, para poder establecer relaciones entre ellas así como rutas de aprendizaje.

%%
%Conocer el negocio y los datos que nos interesen una vez fijados los objetivos..

%Comprobar si las métricas que las sustentan suponen una dificultad incremental, en base a poblaciones
%%%%
% Extraer la mayor cantidad de insights corroborada por una cantidad de proyectos amplia
...
% Detectar para cada proyecto de Snap! Bad Smells e incluirlo en Dr. Snap! Poder comparar con los resultados obtenidos en Dr. Scratch. 
% Extraer los datos de los proyectos de manera automática periódicamente desde el servidor empleando web scrapping. y exponer los resultados más relevantes en el servicio web de Dr-. Scratch
% Poner los resultados generales en la página de Dr. Scratch con filtros, para conocer las estadísticas generales de los proyectos.
% Otros tipos de agrupamiento para contrastar y extraer insights
